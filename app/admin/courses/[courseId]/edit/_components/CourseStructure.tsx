...
export const CourseStructure = ({ data }: Props) => {
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    if (activeType === "chapter") {
      ...
      if (courseId) {
        const chaptersUpdate = updatedChapterForState.map((chapter) => ({
          id: chapter.id,
          position: chapter.order,
        }));

        const reorderPromise = () => reorderChapter(courseId, chaptersUpdate);
    ...
    if (activeType === "lesson" && overType === "lesson") {
      ...
      if (!chapterId || chapterId !== overChapterId) {
        toast.error(
          "Lesson move between different chapters or invalid chapt ID is not allowed.",
        );
        return;
      }
      ...
      if (chapterIndex === -1) {
        toast.error("Could not found chpater for lesson");
        return;
      }
      ...
      if (courseId) {
        ...
        const reorderLessonsPromises = () =>
          reorderLesson(chapterId, lessonsToUpdate, courseId);

        toast.promise(reorderLessonsPromises, {
    ...
    >
      <Card>
        <CardHeader className="flex flex-row items-center justify-between border-b border-border">
          <CardTitle className="">Chapters</CardTitle>
        </CardHeader>
        <CardContent>
